...
class CustomerController extends CI_Controller
{
    ...
    public function getDocumentCategory()
    {
        ...
        if ($category_id) {
            $items = $this->Users_model->documentItemSelect($category_id);
        }
    ...
    public function getPackageCategory()
    {
        ...
        $category_id = $this->input->post('category_id');
        if ($category_id) {
            $items = $this->Users_model->packageItemSelect($category_id);
        }
        echo json_encode($items);
    }
    ...
    public function getShipmentChanges()
    {
        ...
        $rates = $this->Users_model->shipmentRate($ship_cat_id, $ship_subcat_id, $rate_type, $location_from, $location_to);
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!empty($ids)) {
                foreach ($ids as $key => $value) {
                    ...
                    $quot_data = [
                        ...
                        'parent_id'       => $ins_id,
                        'customer_id'    => $customer_id,
                        'shipment_type'  => $shipment_type_option_,
                        'location_type'  => $post['location_type'],
                        'road'           => $road_input_,
                        'rail'           => $rail_input_,
                        'air'            => $air_input_,
                    ...
                    $multi_ins_id = $this->OuthModel->insertQuery('quotation_master', $this->OuthModel->xss_clean($quot_data));
                }
            }
            ...
            if ($quote_type == 1) {
                ...
                if ($ins_id != false) {
                    ...
                    $add_order = $this->customer_model->saveOrder($orderno, $ins_id, $payment_mode, 1);
    ...
    public function onSaveOrder()
    {
        ...
        if ($orderno != '') {
            ...
            $add_order = $this->customer_model->saveOrder($orderno, $quote_id, $payment_mode, 1);
    ...
    public function onGetOrders()
    {
        ...
        $data['profile_details'] = $this->OveModel->Read_User_Information($id);
        $data['orders_list']  = $this->customer_model->getShipmentByUser($id);
        //echo '<pre>';print_r($data['quotation_list']);die;
