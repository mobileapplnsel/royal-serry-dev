...
class Category_model extends CI_Model
{
    ...
    {
        $this->db->insert('document_package_categories', $data);
        return $this->db->insert_id();
    }

    public function editCategory($id)
    {
        $query  =   $this->db->get_where('document_package_categories', array('cat_id' => $id));
        $row    =   $query->num_rows();
        if ($row > 0)
        {
            $row        =   $query->result();
    ...
    public function updateCategory($id, $data)
    {
        $this->db->set($data);
        $this->db->where('cat_id',$id);
        $this->db->update('document_package_categories',$data);
    ...
    {
        $query  =   $this->db->get_where('document_package_categories', array('parent_cat_id' => $id));
        $row    =   $query->num_rows();
        if ($row > 0)
        {
            $row        =   $query->result();
    ...
    {
        $query  =   $this->db->get_where('document', array('category_id' => $id));
        $row = $query->num_rows();
        if ($row > 0)
        {
            return $row;            
    ...
    {
        $query  =   $this->db->get_where('package', array('category_id' => $id));
        $row = $query->num_rows();
        if ($row > 0)
        {
            return $row;            
    ...
    {
        $query  =   $this->db->get_where('document_package_categories', array('category_name' => $name));
        $row    =   $query->num_rows();
		return $row;
        /*if ($row > 0)
        {
    ...
    {
        $query  =   $this->db->get_where('document_package_categories', array('category_name' => $name, 'cat_id !='=> $id));
        $row    =   $query->num_rows();
		return $row;
	}
    ...
    {
        $this->db->delete('document_package_categories', array('cat_id' => $id));      
        return $this->db->affected_rows();
    }
}